...
THC_API void THCudaTensor_conv2Dmv(THCState *state, THCudaTensor *output, float beta, THCudaTensor *input,
                                   THCudaTensor *kernel, int64 srow, int64 scol, const char *type)
{
  THAssert(THCudaTensor_checkGPU(state, 3, output, input, kernel));
  int64 nInputPlane, nInputRows, nInputCols;
  int64 nKernelRows, nKernelCols;
  int64 nOutputPlane, nOutputRows, nOutputCols;
  ...
  if (*type == 'f') {
    ...
    int64 nInputRowsPadded = (nOutputRows - 1) * srow + nKernelRows;
    int64 nInputColsPadded = (nOutputCols - 1) * scol + nKernelCols;
  ...
  int64 nelem = THCudaTensor_nElement(state, output);
...
{
  THAssert(THCudaTensor_checkGPU(state, 3, output, input, kernel));
  int64 nbatch, nInputPlane, nInputRows, nInputCols;
  int64 nKernelRows, nKernelCols;
  int64 nOutputPlane, nOutputRows, nOutputCols;
...
THC_API void THCudaTensor_conv2DRevger(THCState *state, THCudaTensor *output, float beta, float alpha,
                                       ...
                                       int64 srow, int64 scol)
{
  THAssert(THCudaTensor_checkGPU(state, 3, output, input, kernel));
  int64 nInputPlane, nInputRows, nInputCols;
  int64 nKernelPlane, nKernelRows, nKernelCols;
  int64 nOutputRows, nOutputCols;
      ...
      for(oo = oo_start; oo < oo_end; oo++) {
        for (ii = table_start; ii < table_end; ii = ii + 2) {
          for(yy = yy_start; yy < yy_end; yy+=yy_step) {
            for(xx = xx_start; xx < xx_end; xx+=xx_step) {
              // Dot product in two dimensions... (between input image and the mask)
              float *input_p = input + ((int64)table[ii]-1)*input_h*input_w
                + yy*stride_h*input_w + xx*stride_w;
              float *output_p = output + oo*output_h*output_w + yy*output_w + xx;
              float *kernel_p = shared_kernel
                + ((int64)table[ii + 1]-1) *kernel_w*kernel_h + koffset;
              float sum = 0;
              if (swapkernel) {
#pragma unroll
                for(ky = 0; ky < T_kernel_h; ky++) {
        ...
        for(yy = yy_start; yy < yy_end; yy+=yy_step) {
          for(xx = xx_start; xx < xx_end; xx+=xx_step) {
            ...
            float *input_p = input + ((int64)table[ii]-1)*input_h*input_w
              + yy*stride_h*input_w + xx*stride_w;
            float *output_p = output + oo*output_h*output_w + yy*output_w + xx;
            float *kernel_p = kernel + ((int64)table[ii + 1]-1) *kernel_w*kernel_h + koffset;
            float sum = 0;
            if (swapkernel) {
              for(ky = 0; ky < kernel_h; ky++) {
#pragma unroll 5
...
THC_API void THCudaTensor_conv2Dmap(THCState *state, THCudaTensor *output, THCudaTensor *input,
                                    THCudaTensor *kernel, int64 stride_x, int64 stride_y,
                                    THCudaTensor *table, int64 fanin)
{
